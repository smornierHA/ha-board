...
class WeatherCombinedForecastCard extends HTMLElement {
  ...
  _unsubscribeDailyForecast() {
    const unsub = this._dailyForecastUnsubscribe;
    this._dailyForecastUnsubscribe = null;
    this._dailyForecastSubscriptionPending = false;
    if (typeof unsub === "function") {
      try { unsub(); } catch (_err) { /* ignore closed connection */ }
    }
  }
  ...
  _ensureDailyForecastSubscription() {
    ...
    if (this._dailyForecastEntity === entityId && (this._dailyForecastUnsubscribe || this._dailyForecastSubscriptionPending)) return;

    this._unsubscribeDailyForecast();
    this._dailyForecastEntity = entityId;
    this._dailyForecasts = this._weatherLegacyForecastRows();
    ...
    connection.subscribeMessage(
      (event) => {
        if (this.config?.weather_entity !== entityId) return;
        this._dailyForecasts = Array.isArray(event?.forecast) ? event.forecast : [];
      ...
      if (this.config?.weather_entity !== entityId) {
        if (typeof unsubscribe === "function") unsubscribe();
        return;
      }
      this._dailyForecastUnsubscribe = unsubscribe;
    }).catch((err) => {
      // Si le provider ne supporte pas daily, on garde uniquement le legacy forecast éventuel.
      console.debug("weather-combined-forecast-card: daily forecast subscription unavailable", err);
    }).finally(() => {
      if (this.config?.weather_entity === entityId) this._dailyForecastSubscriptionPending = false;
    });
  }
  ...
  renderCard(force = false) {
    ...
    if (!force && renderKey === this._lastRenderKey && this.shadowRoot.querySelector(".combined-scroll")) {
      requestAnimationFrame(() => { this._setupScrollInteractions(); this._setupRiskPillInteractions(); this._setupMainHeaderAction(); });
      return;
    }
