...
`npm run test:garage` exécute 15 contrats de la source candidate avec Lit simulé et tous les appels de service mockés. Ils couvrent G1, réponses tardives après détachement ou reconfiguration, double commande, modes pulse/stateful/cover, confirmation physique distincte du succès de service, timeout, états inconnus, caméra A/B/A, reconnexion avec dialogue ouvert et caméra sélectionnée, annulation de la microtâche caméra après détachement, nettoyage, instances multiples et éditeur natif avec `false`/`0` et clés inconnues. `python3 scripts/check_garage.py` contrôle les quatre empreintes de provenance, la dépendance Lit verrouillée et incorporée, l’absence d’import CDN dans la distribution et l’intégrité Personnes/météo.

`python3 -m unittest tests/test_garage_resource_migration.py -v` rejoue, sur ressources fictives, le corps HACS installé avec Personnes en première position du namespace, météo intacte et Garage déplacé après le namespace avant téléchargement. Il vérifie la bascule et le rollback Garage seuls ainsi qu’une mise à jour ultérieure à trois modules. `node --experimental-websocket tests/garage-browser.mjs` produit dans `artifacts/garage-browser/` une comparaison fictive original/candidat/distribution et vérifie navigation, hash, Escape, reconnexion du dialogue ouvert, unicité de la caméra sélectionnée, invalidation après reconfiguration/détachement, détachement tardif de commande et réouverture de l’éditeur. Ce banc ne constitue ni une recette HA native ni une preuve d’ouverture physique.
