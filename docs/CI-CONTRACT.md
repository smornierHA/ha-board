...
Le workflow installe strictement `package-lock.json`, construit Garage avec `lit-element@4.2.0` incorporé, refuse un CDN/import runtime et vérifie les empreintes original public/export/candidat/distribution. Les 15 contrats source couvrent G1, réponse tardive après détachement/reconfiguration, double commande, modes pulse/stateful/cover, confirmation physique/expiration, inconnus/indisponibles, caméra A/B/A, reconnexion avec dialogue ouvert et caméra sélectionnée, microtâche caméra annulée au détachement, nettoyage, plusieurs instances et éditeur (`false`, `0`, objets et clés inconnues). Tous les services sont des doubles.

Le banc Chrome compare l’export original, la source construite et le module distribué sur données/images fictives, produit les captures et exerce sur source et bundle la reconnexion caméra sans fermer le dialogue, l’unicité de la caméra sélectionnée, l’annulation après reconfiguration/détachement et le rejet d’une erreur tardive. L’égalité visuelle du banc n’est pas une recette native HA. Les tests Python rejouent la méthode HACS installée sur trois ressources HA-BOARD fictives, une mise à jour ultérieure et le rollback Garage seul, sans rejouer la migration météo réelle.
