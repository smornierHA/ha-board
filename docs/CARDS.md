...
L’éditeur propose les groupes Prévisions, Probabilité pluie, Vent, En-tête/actions, Risques et Options avancées. Il affiche les valeurs effectives issues de l’unique définition des défauts, y compris pour un YAML minimal. Les listes de risques et actions utilisent le sélecteur objet natif HA pour conserver leurs clés imbriquées. `false`, `0`, objets, champs inconnus et `grid_options` sont préservés lors d’une modification ciblée ; remettre une option à son défaut retire sa surcharge. Les trois options d’unités historiques sans effet restent acceptées et conservées en YAML, mais ne sont plus modifiables dans le formulaire. Les actions proposées restent celles du composant existant : navigation, more-info, URL, call-service, fire-dom-event, none. Les tests ne commandent aucun appareil.
