...
def main():
    ...
    if len({item['file'] for item in metadata}) != len(metadata):
        raise SystemExit('Duplicate metadata')
    for item in metadata:
        if not re.fullmatch(r'[a-z0-9-]+\.json', item['file']):
            raise SystemExit('Invalid metadata filename')
        data = Path('dist', item['file']).read_bytes()
        if hashlib.sha256(data).hexdigest() != item['sha256'] or len(data) != item['bytes']:
    ...
    if Path('weather-provenance.json').exists():
        ...
        if not set(evidence.get('distributed_notices', [])) <= notice_names:
    ...
    if Path('garage-provenance.json').exists():
        garage = json.loads(Path('garage-provenance.json').read_text())
        dependency = garage.get('dependency_review', {})
        if dependency.get('status') != 'verified' or garage.get('privacy_review') != 'verified-fictitious-only':
            raise SystemExit('Garage dependency/privacy provenance review pending; no release created.')
        if dependency.get('distributed_notices') != [item['file'] for item in notices]:
            raise SystemExit('Garage dependency evidence is not linked to the manifested notices.')
        for key in ['notice', 'license_file', 'lockfile']:
            if not Path(dependency.get(key, '')).is_file():
                raise SystemExit(f'Garage dependency evidence missing: {key}')
        for key in ['public_export', 'candidate_source', 'distribution']:
            record = garage[key]
            data = Path(record['path']).read_bytes()
            if len(data) != record['bytes'] or hashlib.sha256(data).hexdigest() != record['sha256']:
                raise SystemExit(f'Garage provenance digest mismatch: {key}')
    ...
    if existing:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name in [item['file'] for item in artifacts + notices + metadata] + ['manifest.json']:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        run('gh', 'release', 'create', tag, *[str(Path('dist', item['file'])) for item in artifacts + notices + metadata], 'dist/manifest.json', str(proof),
